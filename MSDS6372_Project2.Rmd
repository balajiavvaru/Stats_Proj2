...
for(j in 1:iterations)
{
  ...
  model = naiveBayes(train[,c(1,2)],as.factor(as.factor(train$y)),laplace = 1)
  tabledata= table(predict(model,test[,c(1,2)]),as.factor(test$y))
  
  CM = confusionMatrix(table(predict(model,test[,c(1,2)]),as.factor(as.factor(test$y))))
